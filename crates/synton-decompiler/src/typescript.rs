...
use synton_ast::{Module, Expr, Stmt, Type, ImportDecl, Span};
    ...
    fn gen_type(&self, ty: &Type) -> DecompilerResult<String> {
        Ok(match &ty.kind {
            synton_ast::TypeKind::Builtin(b) => match b {
                synton_ast::BuiltinType::I32 => "number",
                synton_ast::BuiltinType::I64 => "number",
                synton_ast::BuiltinType::F32 => "number",
                synton_ast::BuiltinType::F64 => "number",
                synton_ast::BuiltinType::Bool => "boolean",
                synton_ast::BuiltinType::String => "string",
                _ => "unknown",
            }.to_string(),
            synton_ast::TypeKind::List(inner) => format!("{}[]", self.gen_type(inner)?),
            _ => "unknown".to_string(),
        })
    }
}
...
impl TypeScriptCodegen {
    fn gen_import(&self, buf: &mut CodeBuffer, import: &ImportDecl) -> DecompilerResult<()> {
        buf.push_line(&format!("import {{ {} }} from \"{}\";", import.name, import.name));
        Ok(())
    }
    ...
    fn gen_stmt_buf(&self, buf: &mut CodeBuffer, stmt: &Stmt) -> DecompilerResult<()> {
        match &stmt.kind {
            ...
            synton_ast::StmtKind::Let { name, ty, init, .. } => {
                ...
                let type_ann = if self.config.types {
                    ty.as_ref()
                        .map(|t| format!(": {}", self.gen_type(t).unwrap_or_else(|_| "unknown".to_string())))
                        .unwrap_or_default()
                } else {
                    String::new()
                };
            ...
            synton_ast::StmtKind::Expr(e) => {
                buf.push_line(&format!("{};", self.gen_expr(e)?));
            }
            _ => {}
        }
        Ok(())
    ...
    fn gen_fn_decl(&self, buf: &mut CodeBuffer, decl: &synton_ast::stmt::FnDecl) -> DecompilerResult<()> {
            ...
            .map(|p| {
                if self.config.types {
                    let ty_str = p.ty.as_ref()
                        .map(|t| self.gen_type(t).unwrap_or_else(|_| "unknown".to_string()))
        ...
        if let Some(body) = &decl.body {
            buf.push_line(&format!("return {};", self.gen_expr(body)?));
        }
    ...
    fn gen_literal(&self, buf: &mut String, lit: &synton_ast::expr::Literal) {
        match lit {
            ...
            synton_ast::expr::Literal::Unit => buf.push_str("undefined"),
            _ => {}
        }
    }
}
...
fn unknown_type() -> Type {
    Type::new(synton_ast::TypeKind::Inference(0), Span::new(
